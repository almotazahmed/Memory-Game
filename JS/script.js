...
class MemoryCardGame {
  ...
  constructor(
    ...
    this.firstClickedImgBox;
    this.numberOfCorrectChoose;
    this.numberOfTemps = 0;
    this.flippingTime = 800; // Duration for flipping cards (in milliseconds).
  }
  ...
  updateRoundCounter() {
    window.sessionStorage.setItem("temp-number", ++this.numberOfTemps);
  ...
  firstStart() {
    if (
      window.sessionStorage.getItem("temp-number") &&
      window.sessionStorage.getItem("player-name")
    ) {
      // If session data exists (player name and round count), load it.
      this.numberOfTemps = window.sessionStorage.getItem("temp-number");
      this.tempsNumberBox.textContent = this.numberOfTemps;
  ...
  startPopup() {
    ...
    btn.addEventListener("click", () => {
      ...
      if (!playerName) {
        ...
        window.sessionStorage.setItem("temp-number", this.numberOfTemps); // Store the round count in session storage.
...
let wrongTriesBox = document.querySelector(".wrong-tries span");
let time = 120;
